...
def analyze_energy_mood(audio_path, output_path, song_title, artist=None):
    """
    分析音频文件的能量与情绪，生成综合摘要图表。
    ...
    返回：
        dict: 包含 energy_score, valence_score, arousal_score, mood_label, energy_label 等
    """
    artist_str = f" — {artist}" if artist else ""

    print("=" * 60)
    print(f"  {song_title} — 能量与情绪分析")
    print("=" * 60)
    ...
    fig.suptitle(f"《{song_title}》{artist_str}\n"
    ...
    ax.scatter(x, y, s=500, c="gold", edgecolors="black", linewidth=2,
               zorder=5)
    ax.annotate(song_title, (x, y), textcoords="offset points",
...
if __name__ == "__main__":
    import argparse
    import os
    parser = argparse.ArgumentParser(description="能量与情绪综合分析")
    ...
    analyze_energy_mood(args.audio, args.output, args.title, args.artist)
