...
def analyze_curves(audio_path, output_path, song_title, artist=None, sr=SR, segment_sec=SEGMENT_SEC):
    """
    分析音频文件的能量与情绪随时间变化的曲线，生成时间曲线图表。

    参数：
        audio_path (str): 输入音频文件路径
        output_path (str): 输出 PNG 图表保存路径
        song_title (str): 歌曲名称（用于图表标题）
        artist (str, optional): 艺术家名称（用于图表标题）
        sr (int): 采样率，默认 22050
        segment_sec (int): 每段秒数，默认 2
    ...
    print("=" * 60)
    print(f"  {song_title} — 能量与情绪时间曲线分析")
    print("=" * 60)
    ...
    fig.suptitle(f"《{song_title}》{artist_str}  能量与情绪时间曲线  (BPM: {tempo_global:.0f})",
    ...
    # 情绪走势判断
    if valence_curve[0] > valence_curve[-1] + 0.05:
        mood_trend = "前高后低"
    elif valence_curve[-1] > valence_curve[0] + 0.05:
        mood_trend = "前低后高"
    else:
        mood_trend = "相对平稳"
...
""")
    ...
    return {
        ...
        "valence_std": float(np.std(valence_curve)),
        "arousal_mean": float(np.mean(arousal_curve)),
        "arousal_std": float(np.std(arousal_curve)),
        "high_energy_ratio": float(np.sum(chorus_mask) / len(chorus_mask)),
        "tempo_bpm": tempo_global,
...
if __name__ == "__main__":
    import argparse
    import os
    parser = argparse.ArgumentParser(description="能量与情绪时间曲线分析")
    parser.add_argument("audio", help="输入音频文件路径")
    parser.add_argument("--output", "-o", required=True, help="输出 PNG 图表路径")
    parser.add_argument("--title", "-t", required=True, help="歌曲名称")
    parser.add_argument("--artist", "-a", default=None, help="艺术家名称")
    parser.add_argument("--sr", type=int, default=22050, help="采样率 (默认 22050)")
    parser.add_argument("--segment", type=int, default=2, help="每段秒数 (默认 2)")
    args = parser.parse_args()

    # 确保输出目录存在
    os.makedirs(os.path.dirname(args.output), exist_ok=True)

    analyze_curves(args.audio, args.output, args.title, args.artist,
                   sr=args.sr, segment_sec=args.segment)
